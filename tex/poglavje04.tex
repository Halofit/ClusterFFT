...
Podatki kažejo na linearno pohitritev glede na število vozlišè in linearno poveèanja èasa glede na število datotek.
...
Maksimalna hitrost obdelave podatko je veliko manjša pri veè majhnih datotek kot pri eni veliki. Zanimiva je primerjava s tabelo \ref{hitrost1}.
...
Uspešno smo sesuli strežnik tako, da smo nanj povezali 10 klientov hkrati. Vsak klient je poslal eno datoteko. Strežnik je datoteke sprejemal, nato pa je nekatere povezave zaprl, nekatere pa so se izvedle uspešno. Kot nadaljne delo bi bilo zanimivo ugotoviti, zakaj se je to zgodilo in kako bi to prepreèili, saj za razlog nismo preprièani. Sumimo, da je zaradi neuspešnega klica sistemskega klica malloc (rezervacija spomina) prišlo do pisanja po nedovoljenem spominu.

\subsection{Ugotovitve}

Na podlagi vseh izmerjenih podatkov smo ugotovili, da je Beowulf gruèa, sestavljena iz raèunalnikov Raspberry Pi ugodna rešitev glede na razmerje med ceno in zmogljivostjo. Zaradi zelo majhnega delovnega pomnilnika (512MB, model 2 verzija B+ pa 1GB) ni primerna za reševanje velikih problemov. Pri doloèenih problemih, naprimer našem, je ta težava rešljiva s pazljivim programiranjem, pri mnogih pa ni.

Ker je pri uporabi gruèe vedno prisotna režija (predvsem porazdeljevanje ali kopiranje podatkov po gruèi), je en zelo zmogljiv sistem boljši od gruèe majhnih raèunalnikov, vendar na raèun visoke cene. Razlika je oèitna - na enem raèunalniku je èas režije 20 do 40krat manjši kot na gruèi (glej gref \ref{fig:mpiovermany}).

Ko smo testirali z veè instancami klienta hkrati, so se pojavile mnoge težave na našem strežniku. Mnogo klientov je prikinil, pri 10 ali veè klientih pa se je strežnik sesul. To nam pove, da naša konfiguracija ni primerna za uporabo v realnem svetu kot oddaljeno storitev. To je sicer težava naše implementacije in gruèe ali raèunalnikov Raspberry Pi.
